...
public class LogManager {
	...
	public static void registrarCambioProducto(String nombreEmpleado, String idProducto, String campoModificado, String antes, String despues) {
		String mensaje = obtenerFechaHora() + " | Empleado: " + nombreEmpleado + " | ProID: " + idProducto + " | Modificado [" + campoModificado + "] de '" + antes + "' a '" + despues + "'";
		escribirLinea(LOG_CAMBIOS, mensaje);
	}
	
	//registra cuando se hace restock
	public static void registrarRestock(String nombreEmpleado, String idProducto, int cantidadAgregada, int nuevoStockTotal) {
		String mensaje = obtenerFechaHora() + " | Empleado: " + nombreEmpleado + " | ProdID" + idProducto + " | Agrego +" + cantidadAgregada + "unidades (Stock final: " + nuevoStockTotal + ")";
	...
	private static void escribirLinea(String nombreArchivo, String lineaTexto) {
		try {
			//true para que añada la linea al final, que no sobreescriba
			...
			System.out.println("Error al guardar en ka bitacora (" + nombreArchivo + "):" + e.getMessage());
